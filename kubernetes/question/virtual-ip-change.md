...
当机器ubuntu238的apiserver故障，那么238机器的权重减少11变为110。

这个时候ubuntu239的权重最高，ip在它这里。
...
- 查看ubuntu238的keepalived日志：`journalctl -u keepalived`
  ...
  # 刚开始ubuntu238机器的权重为110，服务起来后，设置权重为121
  ...
  ....
  # VRRP_Script：CheckK8sMaster执行失败，把权重设置为110了
...
当机器ubuntu238故障后，`ubuntu239`的apiserver也故障，那么239机器的权重减少12变为100。

这个时候`ubuntu240`的权重最高，IP在它这里。
...
>
> 此时`ubuntu238`的权重110最高，ip继续回到了`ubuntu238`。
...
**注意：**此时虽然三台机器的服务都挂了，但是IP还是会切换，切换到权重值高的。
